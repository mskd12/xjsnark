...
public class AES128Encrypt {
  ...
  private UnsignedInteger[] cipher = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{16}, 8);
  ...
  public AES128Encrypt(UnsignedInteger[] k) {
    for (int i = 0; i < key.length; i++) {
      key[i].assign(k[i], 8);
    }
  }

  public UnsignedInteger[] encrypt(UnsignedInteger[] plaintext) {
    assert plaintext.length == 16;
    for (int i = 0; i < plaintext.length; i++) {
      plaintext[i].assign(plaintext[i], 8);
    }
    ...
    state = subState(state);
    state = shiftRows(state);
    state = addRoundkey(state, nr * Deco_GCM.nb * 4, (nr + 1) * Deco_GCM.nb * 4);

    idx = 0;
    for (int j = 0; j < 4; j++) {
      for (int k = 0; k < 4; k++) {
        cipher[idx].assign(state[k][j], 8);
        idx++;
      }
    }
    return cipher;
  ...
  private UnsignedInteger[] expandKey() {
    ...
    int nr = Deco_GCM.nk + 6;
    UnsignedInteger[] expandedKey = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{Deco_GCM.nb * (nr + 1) * 4}, 8);
    UnsignedInteger[][] w = (UnsignedInteger[][]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{Deco_GCM.nb * (nr + 1), 4}, 8);
    UnsignedInteger[] tmp;
    ...
    while (i < Deco_GCM.nb * (nr + 1)) {
      ...
      for (int v = 0; v < 4; v++) {
        w[i][v].assign(w[i - Deco_GCM.nk][v].xorBitwise(tmp[v]), 8);
      }
      i++;
    }
    int idx = 0;

    for (int k = 0; k < Deco_GCM.nb * (nr + 1); k++) {
      for (int j = 0; j < 4; j++) {
  ...
  private UnsignedInteger gal_mul_const(UnsignedInteger x, int c) {
    ...
    for (int counter = 0; counter < 8; counter += 1) {
      ...
      {
        Bit bit_l0c0x = hi.copy();
        boolean c_l0c0x = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_l0c0x);
        if (c_l0c0x) {
          if (bit_l0c0x.getConstantValue()) {
            x.assign(tmp, 8);
          } else {
          ...
          ConditionalScopeTracker.push(bit_l0c0x);
          x.assign(tmp, 8);
