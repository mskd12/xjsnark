...
public class AES128Encrypt {
  ...
  private UnsignedInteger gal_mul_const(UnsignedInteger x, int c) {
    ...
    for (int counter = 0; counter < 8; counter += 1) {
      ...
      {
        Bit bit_l0c0ab = hi.copy();
        boolean c_l0c0ab = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_l0c0ab);
        if (c_l0c0ab) {
          if (bit_l0c0ab.getConstantValue()) {
            x.assign(tmp, 8);
          } else {
          ...
          ConditionalScopeTracker.push(bit_l0c0ab);
          x.assign(tmp, 8);
